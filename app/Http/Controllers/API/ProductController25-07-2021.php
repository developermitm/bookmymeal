...
class ProductController extends Controller
{
    ...
    {   
        ...
        $colorVariants = Products::select('product.id','product.title','product.current_stock','product.discount','product.sale_price','stock.color','stock.size','stock.quantity')->leftJoin('stock', 'product.id', '=', 'stock.product_id')->
            ...
            if(isset($sizeVariants) && count($sizeVariants) > 0){
                foreach($sizeVariants as $variant){   
                    ...
                    if(trim(strtolower($variant->color)) == trim(strtolower($colorvar))){
            ...
            if(isset($colorVariants) && count($colorVariants) > 0){
               foreach($colorVariants as $variant){                                        
                    if(trim( strtolower($variant->size))  == trim( strtolower($products->size)) ){
                        if($variant->id != $products->id){
                            $stock = ($variant->current_stock > 0) ? $variant->current_stock : 0;
                            $colorArr[]= array('id' => $variant->id, 'color' => $variant->color, 'title' =>$variant->title,'sale_price'=>$variant->sale_price, 'discount'=>$variant->discount,'size'=>$variant->size,'stock'=> $stock);
                        }
                    } 
                }
            }  
        ...
        $sizeColorArr = array('color' => $colorArr, 'size' => $size);
    ...
    public function wishListing(){
        $userId = Auth::user()->id;
        $getCartItems['wishListItems'] = Wishlist::select('product.id','product.title','product.sale_price','product.current_stock','product.purchase_price','product.main_image as thumbnail')->
    ...
    public function bannerList(){
        ...
        $toTime = time();
        $products['bannerList'] = Banner::select('banner.page', 'ban
            ner.place', 'banner.num', 'banner.image_ext', 'product.title')->
    ...
    public function orderList(){
        $dataArr = [];
        $userId = Auth::user()->id;
        $getOrder = Order::where(['buyer' => $userId ])->get();
        if(!empty($getOrder)){
